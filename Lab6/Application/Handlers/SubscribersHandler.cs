...
public class SubscribersHandler(ApplicationDbContext context) :
    ...
    IRequestHandler<DeleteSubscriberRequest>,
    IRequestHandler<GetSubscriberMailingListItemsRequest, GetItemsResponse<SubscriberMailingListModel>>,
    IRequestHandler<AddMailingListToSubscriberRequest>, 
    IRequestHandler<DeleteMailingListFromSubscriberRequest>
{
    ...
    public async Task Handle(DeleteSubscriberRequest request, CancellationToken cancellationToken)
    {
        var entity = await context.Subscribers
            .FirstOrDefaultAsync(x => x.Id == request.Id, cancellationToken);
    ...
    public async Task Handle(AddMailingListToSubscriberRequest request, CancellationToken cancellationToken)
    {
    ...
    public async Task Handle(DeleteMailingListFromSubscriberRequest request, CancellationToken cancellationToken)
    {
        var entity = await context.MailingListSubscribers
            .FirstOrDefaultAsync(x => x.SubscriberId == request.Id && x.MailingListId == request.MailingListId, cancellationToken);
